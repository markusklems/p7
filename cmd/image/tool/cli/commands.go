...
type (
	...
	// CreateImageCommand is the command line data structure for the create action of image
	CreateImageCommand struct {
		Payload     string
		ContentType string
		PrettyPrint bool
	}

	// DeleteImageCommand is the command line data structure for the delete action of image
	DeleteImageCommand struct {
		// Image ID
		ImageID     string
		PrettyPrint bool
	}
...
func RegisterCommands(app *cobra.Command, c *client.Client) {
	...
	command = &cobra.Command{
		Use:   "create",
		Short: `Create new image`,
	}
	tmp2 := new(CreateImageCommand)
	sub = &cobra.Command{
...

{
   ...
   "tag": "Provident repellendus ut corrupti."
	...
	tmp3 := new(DeleteImageCommand)
	sub = &cobra.Command{
		Use:   `image ["/image/images/IMAGE_ID"]`,
		Short: ``,
	...
	command = &cobra.Command{
		Use:   "list",
		Short: `Retrieve all images.`,
	}
	tmp4 := new(ListImageCommand)
	sub = &cobra.Command{
		Use:   `image ["/image/images"]`,
		Short: ``,
	...
	command = &cobra.Command{
		Use:   "show",
		Short: `Retrieve image with given id. IDs 1 and 2 pre-exist in the system.`,
	}
	tmp5 := new(ShowImageCommand)
	sub = &cobra.Command{
		Use:   `image ["/image/images/IMAGE_ID"]`,
		Short: ``,
...
// Run makes the HTTP request corresponding to the CreateImageCommand command.
func (cmd *CreateImageCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
	...
	var payload client.CreateImagePayload
	if cmd.Payload != "" {
	...
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.CreateImage(ctx, path, &payload, cmd.ContentType)
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
// RegisterFlags registers the command flags with the command line.
func (cmd *CreateImageCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	cc.Flags().StringVar(&cmd.Payload, "payload", "", "Request body encoded in JSON")
	cc.Flags().StringVar(&cmd.ContentType, "content", "", "Request content type override, e.g. 'application/x-www-form-urlencoded'")
}

// Run makes the HTTP request corresponding to the DeleteImageCommand command.
func (cmd *DeleteImageCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
		path = fmt.Sprintf("/image/images/%v", url.QueryEscape(cmd.ImageID))
	}
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.DeleteImage(ctx, path)
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
// RegisterFlags registers the command flags with the command line.
func (cmd *DeleteImageCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	var imageID string
	cc.Flags().StringVar(&cmd.ImageID, "image_id", imageID, `Image ID`)
}
